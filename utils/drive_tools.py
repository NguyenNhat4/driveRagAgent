...
def get_credentials() -> Optional[Credentials]:
    ...
    if os.path.exists(TOKEN_FILE):
        try:
            ...
            # Verify scopes - strict set comparison might be too much if order differs,
            # but checking if required scopes are a subset of granted scopes is safer.
            # However, from_authorized_user_file doesn't auto-check.
            # Google's Credentials object has `has_scopes` method.
            if not creds.has_scopes(SCOPES):
                 logger.warning("Token scopes mismatch. Deleting token.json to re-authenticate.")
                 creds = None
